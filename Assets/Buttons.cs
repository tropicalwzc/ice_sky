...
public class Buttons : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(5, Screen.height - proper_big_button_size, proper_big_button_size, proper_big_button_size), btnretry, gooder) || Input.GetKeyDown(KeyCode.R) && sander > 10)
        {
            sander = 0;
            SceneManager.LoadScene(restart_scene);
        }

        if (GUI.Button(new Rect(5 + proper_big_button_size, Screen.height - proper_big_button_size, proper_big_button_size, proper_big_button_size), backtomenu, gooder) || Input.GetKeyDown(KeyCode.R) && sander > 10)
        {
            sander = 0;
